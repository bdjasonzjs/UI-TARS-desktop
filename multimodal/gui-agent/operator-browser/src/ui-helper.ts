...
export class UIHelper {
  ...
  async showWaterFlow() {
    try {
      await this.showWaterFlowInner();
    } catch (error) {
      this.logger.error('Failed to show water flow effect:', error);
    }
  }

  private async showWaterFlowInner() {
    ...
    await page.evaluate((waterFlowId: string) => {
      const WATERFLOW_STYLE_TXT_CONTENT = (waterFlowId: string): string => `
#${waterFlowId}::before {
  content: "";
  position: fixed;
...
`;
      try {
        ...
        waterFlow.style.cssText = WATERFLOW_STYLE_CSS_TXT;

        const style = document.createElement('style');
        style.textContent = WATERFLOW_STYLE_TXT_CONTENT(waterFlowId);

        document.head.appendChild(style);
        document.body.appendChild(waterFlow);
      } catch (error) {
        console.error('Failed to showWaterFlowInner on evaluate:', error);
      }
    }, this.waterFlowId);
  ...
  async hideWaterFlow() {
    try {
      await this.hideWaterFlowInner();
    } catch (error) {
      this.logger.error('Failed to hide water flow effect:', error);
    }
  }

  private async hideWaterFlowInner() {
  ...
  async highlightClickableElements() {
    try {
      await this.highlightClickableElementsInner();
    } catch (error) {
      this.logger.error('Failed to highlight clickable elements:', error);
    }
  }

  private async highlightClickableElementsInner() {
...
`;
      try {
        ...
        const createLegend = () => {
          ...
          legend.innerHTML = CLICK_LEGEND_INNER_HTML;
        ...
        console.error('Error highlighting clickable elements:', error);
      }
    }, this.highlightClass);
  ...
  async removeClickableHighlights() {
    try {
      await this.removeClickableHighlightsInner();
    } catch (error) {
      this.logger.error('Failed to remove clickable highlights:', error);
    }
  }

  private async removeClickableHighlightsInner() {
    ...
    await page.evaluate(
      (params) => {
        try {
          ...
          console.error('Error showing action info:', error);
        }
      },
  ...
  async showClickIndicator(x: number, y: number) {
    try {
      await this.showClickIndicatorInner(x, y);
    } catch (error) {
      this.logger.error('Failed to show click indicator:', error);
    }
  }

  private async showClickIndicatorInner(x: number, y: number) {
    ...
    await page.evaluate(
      ...
      ({ x, y, containerId }) => {
        try {
          ...
          console.error('Error showing click indicator:', error);
        }
      },
   ...
   */
  async showDragIndicator(startX: number, startY: number, endX: number, endY: number) {
    try {
      await this.showDragIndicatorInner(startX, startY, endX, endY);
    } catch (error) {
      this.logger.error('Failed to show drag indicator:', error);
    }
  }

  private async showDragIndicatorInner(startX: number, startY: number, endX: number, endY: number) {
    ...
    await page.evaluate(
      ...
      ({ startX, startY, endX, endY, containerId }) => {
        try {
          ...
          console.error('Error showing drag indicator:', error);
        }
      },
